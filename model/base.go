...
func init() {
	...
	dbUri := fmt.Sprintf("host=%s user=%s dbname=%s currentSchema=example011 sslmode=disable password=%s", dbHost, username, dbName, password) //Создать строку подключения
	fmt.Println(dbUri)
/*
